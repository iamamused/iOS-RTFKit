...
int ecEndGroupAction(rkDestinationState rds);
int ecApplyPropChange(rkProperties iprop, int val);
int ecChangeDest(rkDestinationType idest);
int ecParseSpecialKeyword(rkImageTypes ipfn);
int ecParseSpecialProperty(rkProperties iprop, int val);
int ecParseHexByte(void);

// RTF variable declarations

extern int cGroup;
extern rkDestinationState rds;
extern rkInternalState ris;

extern rkCharacterProperities chp;
extern rkParagraphProperities pap;
extern rkSectionProperities sep;
extern rkDocumentProperities dop;

extern rkSavePropertyState *psave;
extern long cbBin;
extern long lParam;
